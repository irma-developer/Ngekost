...
@section('content')
    ...
    <div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
        ...
        <a href="#" class="w-12 h-12 flex items-center justify-center shrink-0 rounded-full overflow-hidden bg-white">
            <img src="{{ asset('storage/assets/images/icons/notification.svg') }}" class="w-[28px] h-[28px]" alt="icon">
        </a>
    </div>
            ...
            @foreach ($categories as $category)
                <div class="swiper-slide w-fit pb-[30px]">
                    <a href="{{ route('category.show', $category->slug) }}" class="card">
                        <div class="flex flex-col items-center w-[120px] shrink-0 rounded-[40px] p-4 pb-5 gap-3 bg-white">
        ...
        <div class="flex items-center justify-between px-5">
            ...
            <a href="#">
                <div class="flex items-center gap-2">
                    <span>See all</span>
                    <img src="{{ asset('storage/assets/images/icons/arrow-right.svg') }}" class="w-6 h-6 flex shrink-0"
                        alt="icon">
                </div>
            </a>
        </div>
                            ...
                            <div
                                ...
                                <div class="flex flex-col gap-3">
                                    ...
                                    <div class="flex items-center gap-[6px]">
                                        <img src="{{ asset('storage/assets/images/icons/location.svg') }}"
                                            class="w-5 h-5 flex shrink-0" alt="icon">
                                        <p class="text-sm text-ngekos-grey">{{ $boardingHouse->city->name }}</p>
                                    </div>
                                    <div class="flex items-center gap-[6px]">
                                        <img src="{{ asset('storage/assets/images/icons/3dcube.svg') }}"
                                            class="w-5 h-5 flex shrink-0" alt="icon">
                                        <p class="text-sm text-ngekos-grey">In {{ $boardingHouse->category->name }}</p>
                                    </div>
                                    <div class="flex items-center gap-[6px]">
                                        <img src="{{ asset('storage/assets/images/icons/profile-2user.svg') }}"
                                            class="w-5 h-5 flex shrink-0" alt="icon">
